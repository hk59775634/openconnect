...
void prepare_script_env(struct openconnect_info *vpninfo)
{
	...
	/* SPEC-02 / ocserv-tunnel: Dynamic Split Tunnel domain lists */
	if (vpninfo->ip_info.dynamic_split_includes) {
		char *list;
		int len = 0;
		struct oc_split_include *d = vpninfo->ip_info.dynamic_split_includes;

		while (d) {
			len += strlen(d->route) + 1;
			d = d->next;
		}
		list = malloc(len);
		if (list) {
			...
			d = vpninfo->ip_info.dynamic_split_includes;
			while (1) {
				strcpy(p, d->route);
				p += strlen(p);
				d = d->next;
			...
			script_setenv(vpninfo, "CISCO_DYNAMIC_SPLIT_INCLUDE_DOMAINS",
				      list, 0, 0);
			free(list);
		}
	} else {
		script_setenv(vpninfo, "CISCO_DYNAMIC_SPLIT_INCLUDE_DOMAINS", NULL, 0, 0);
	}

	if (vpninfo->ip_info.dynamic_split_excludes) {
		char *list;
		int len = 0;
		struct oc_split_include *d = vpninfo->ip_info.dynamic_split_excludes;
		...
		if (list) {
			...
			d = vpninfo->ip_info.dynamic_split_excludes;
			while (1) {
				strcpy(p, d->route);
				p += strlen(p);
				d = d->next;
			...
			script_setenv(vpninfo, "CISCO_DYNAMIC_SPLIT_EXCLUDE_DOMAINS",
				      list, 0, 0);
			free(list);
		}
	} else {
		script_setenv(vpninfo, "CISCO_DYNAMIC_SPLIT_EXCLUDE_DOMAINS", NULL, 0, 0);
	}
